...
"""
DETR (panoptic) evaluation over the COCO subset.

DETR represents the transformer-with-bipartite-matching architecture family —
no anchor boxes, no NMS, end-to-end set prediction. The panoptic head produces
both instance ("thing") and semantic ("stuff") segments; we retain only the
thing segments for fair comparison with Mask R-CNN and Mask2Former.

CLI:
    python src/experiments/run_detr.py \
        --subset data/subsets/subset_200.json \
        --output results/detr_results.json \
...
from src.models.detr_predictor import DETRPredictor
...
def run(args):
    ...
    model = DETRPredictor(device=device, score_threshold=args.score_threshold)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output",           default="results/detr_results.json")
